...
app.config['MAIL_USERNAME'] = '.com' #ganti pake email sendiri
app.config['MAIL_PASSWORD'] = '' 
app.config["JWT_SECRET_KEY"] = os.getenv('JWT_SECRET_KEY')
app.config['MAIL_DEFAULT_SENDER'] = '@gmail.com' #ganti pake email sendiri
...
def edit_profile():
    try:
        ...
        # Menyimpan foto dengan nama berdasarkan username
        if photo:
            filename = secure_filename(photo.filename)
            photo_filename = f"{username}_{filename}"  # Gunakan nama username sebagai bagian dari nama file
        ...
        # Update data user lainnya jika ada
...
def profile():
    try:
        ...
        profile_data = {
            ...
            'photo': url_for('static', filename='uploads/' + user_data.get('photo', 'default.jpg'))
        }
...
def forgot_password():
    try:
        ...
        # Update password in MongoDB
        mongo.db.users.update_one({"email": email}, {"$set": {"password": new_password_hash}})
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        user_data = User.find_by_email(user_data['email'])
        try:
            msg = Message('Password Changed', recipients=[user_data['email']])
            msg.body = 'Your password has been successfully changed.'
            mail.send(msg)
            logging.debug("Password change confirmation email sent")
        except Exception as e:
...
def protected():
    ...
    # Membuat URL gambar profil
    profile_picture = user_data.get('photo', '')
    if profile_picture:
        profile_picture_url = url_for('static', filename='uploads/' + profile_picture, _external=True)
    else:
        profile_picture_url = url_for('static', filename='uploads/default.jpg', _external=True)  # Menggunakan gambar default jika tidak ada gambar
